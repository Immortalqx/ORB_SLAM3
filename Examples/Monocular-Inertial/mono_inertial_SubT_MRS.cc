                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while (!fTimes.eof())
    {
        ...
        if (!s.empty())
        {
            stringstream ss;
            ss << s;
            vstrImages.push_back(strImagePath + "/" + to_string(idx) + ".png");
            double t;
            ss >> t;
...
LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    while (!fImu.eof())
    {
        ...
        if (!s.empty())
        {
            ...
            vAcc.push_back(cv::Point3f(data[8], data[9], data[10]));
            vGyro.push_back(cv::Point3f(data[5], data[6], data[7]));
        }
    }
}
